 ...
 * <p>
 * 패키지 구조 AuthenticationFailureHandle(interface) -&gt; SimpleUrlAuthenticationFail(class) -&gt;
 * AuthFailHandler 우리는 AuthenticationFilureHandler 구현해야 하지만 기존에 구현이 되었는 SimpleUrlAuthenticationFail
 * 상속받아 응답 메시지와 페이지 경로를 설정할 수 있게 하도록 재정의를 하는 것이다. 페이지 경로와 커스텀을 할 수 있도록 만들어주는 메서드는
 * setDefaultFailureUrl("경로") 메서드 이다.
 */
